...
struct inode *exofs_iget(struct super_block *const sb, const unsigned long ino)
{
	...
	ret = nvme_submit_sync_kv_read(EXOFS_SB(sb)->ns,
				       ...
				       sizeof(fcb), 0);
	if (ret != sizeof(fcb))
		exofs_pr_debug("exofs iget read size: %ld, fcbsize: %ld\n", ret,
			  (long)(sizeof(fcb)));
	if (ret < 0)
		goto bad_inode;
...
{
	exofs_pr_debug("EXOFS EVICT ino=%lu nlink=%u count=%d\n", inode->i_ino,
		  inode->i_nlink, atomic_read(&inode->i_count));

	truncate_inode_pages_final(&inode->i_data);

	if (!inode->i_nlink && !is_bad_inode(inode)) {
		exofs_pr_debug("EXOFS EVICT DELETE ino=%lu nlink=%u count=%d\n",
...
static int exofs_read_folio(struct file *file, struct folio *folio)
{
	...
	void *kaddr;
	ssize_t readlen;
	exofs_pr_debug("exofs read folio: %p\n", folio);
	...
	exofs_pr_debug("READ: page=%lu readlen=%ld first_rec_len=%u "
		  "first_inode=%llu\n",
		  folio->index, readlen,
