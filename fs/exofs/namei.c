...
static int exofs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	exofs_pr_debug("UNLINK BEFORE: ino=%lu nlink=%u count=%d dentry=%px\n",
       inode->i_ino,
       inode->i_nlink,
	...
	exofs_pr_debug("UNLINK AFTER DELETE: ino=%lu nlink=%u count=%d\n",
       inode->i_ino,
       inode->i_nlink,
       atomic_read(&inode->i_count));
	...
	exofs_pr_debug("UNLINK AFTER DROP: ino=%lu nlink=%u count=%d\n",
       inode->i_ino,
       inode->i_nlink,
       atomic_read(&inode->i_count));
			 ...
			 struct dentry *dentry, const char *symname)
{
	...
	if (len <= sizeof(EXOFS_I(inode)->i_data)) {
		exofs_pr_debug("exofs_symlink: fast symlink, len=%zu, symname=%s\n", len, symname);
		 ...
		 * through exofs_aops.
		 */
		exofs_pr_debug("exofs_symlink: slow symlink, len=%zu, symname=%s\n", len, symname);
