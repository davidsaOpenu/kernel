...
{
	exofs_dbg("exofs_clear_entry: de=%p, rec_len=%hu\n", de, le16_to_cpu(de->rec_len));
	de->inode_no = 0;
...
static bool exofs_check_folio(struct folio *folio, void *kaddr)
{
	...
	for (offset = 0; offset + EXOFS_DIR_REC_LEN(1) <= limit;) {
		...
		if (rec_len & 3) {
			exofs_dbg("exofs_check_folio: rec_len=%u & 3 != 0\n",
				  rec_len);
...
bad:
	...
	exofs_msg(sb, KERN_ERR, "corrupt directory inode %lu page %lu (%s)",
		  dir->i_ino, folio->index, reason);
...
static int exofs_readdir(struct file *file, struct dir_context *ctx)
{
	...
	exofs_dbg("exofs readdir pos: %ld\n", (long)pos);
	...
	for (; page_nr < npages; page_nr++, offset = 0) {
		...
		if (IS_ERR(kaddr)) {
			exofs_msg(sb, KERN_ERR,
				  "cannot read directory page %lu: %ld",
				  page_nr, PTR_ERR(kaddr));
