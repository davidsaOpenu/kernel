...
static int exofs_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	if (sb_read_res < 0) {
		ret = sb_read_res;
		exofs_msg(sb, KERN_ERR, "error: failed to read superblock");
		goto cleanup_sb;
	}

	if (le16_to_cpu(fscb.s_magic) != EXOFS_SUPER_MAGIC) {
		exofs_msg(sb, KERN_ERR,
			  "error: bad magic value %hu, read_res: %ld\n",
	...
	if (le32_to_cpu(fscb.s_version) != EXOFS_FSCB_VER) {
		exofs_msg(sb, KERN_ERR, "error: unsupported fscb version %d\n",
	...
	if (IS_ERR(root)) {
		ret = PTR_ERR(root);
		exofs_msg(sb, KERN_ERR, "error: failed to get root inode\n");
		goto cleanup_sb;
	}
	...
	if (!sb->s_root) {
		exofs_msg(sb, KERN_ERR, "error: get root inode failed\n");
		ret = -ENOMEM;
		goto cleanup_root;
	}
...
static int exofs_sync_fs(struct super_block *sb, int wait)
{
	...
	if (!sbi->dirty) {
		...
		exofs_dbg("exofs_sync_fs: no changes to sync in superblock\n");
		return 0;
	}
	...
	write_res = nvme_submit_sync_kv_write(EXOFS_SB(sb)->ns, EXOFS_SUPER_ID,
					      &fscb, sizeof(fscb), 0);
	if (write_res < 0) {
		exofs_msg(sb, KERN_ERR, "error: failed to write superblock");
		spin_lock(&sbi->lock);
		sbi->dirty = true;
		spin_unlock(&sbi->lock);
		return write_res;
